...
const isValidDomain = (domain) => {
	...
	return labels.every(
		(label) =>
			label.length >= 1 &&
			label.length <= EMAIL_RULES.LABEL_MAX &&
			REGEX.DOMAIN_LABEL.test(label)
	);
};

const isNotDisposable = (domain) =>
...
const emailSchema = z
	...
	.superRefine((email, ctx) => {
		...
		if (!isValidLocalPart(local)) {
			ctx.addIssue({
				code: 'custom',
				message: "Email local part is invalid — avoid leading/trailing dots, consecutive dots, or unsupported characters",
			});
		}
...
const usernameSchema = z
	...
	.regex(REGEX.USERNAME, "Username may only contain lowercase letters, numbers, dots, underscores, or hyphens")
